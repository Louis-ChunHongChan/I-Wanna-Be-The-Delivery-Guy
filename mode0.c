...
// Returns the smallest collision map entry found at the four corners of a shape using the specified col and row shift.
...
int collisionLadderCheck(unsigned char *collisionMap, int mapWidth, int col, int row, int width, int height, int colShift, int rowShift) {
    ...
    // check if the corners are 2 return true, else return false
    if (collisionMap[OFFSET(col + colShift, row + rowShift, mapWidth)] == 2 || collisionMap[OFFSET(col + width - 1 + colShift, row + rowShift, mapWidth)] == 2 || collisionMap[OFFSET(col + colShift, row + height - 1 + rowShift, mapWidth)] == 2 || collisionMap[OFFSET(col + width - 1 + colShift, row + height - 1 + rowShift, mapWidth)] == 2) {
        return 1;
    }
    else {
        return 0;
